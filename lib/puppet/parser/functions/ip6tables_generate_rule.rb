...
EOS
    ...
    opt = function_iptables_parse_options( [ options, defaults, '6' ] )
    flg = opt['mod_flags']
    flg.default=false

    # addresses are arrays that should always have at least one object, even if
    # its an empty-string
    dst = function_split_ip_by_version( [ opt['destination'] ] )['6']
    dst.push('') if dst.size == 0
    src = function_split_ip_by_version( [ opt['source'] ] )['6']
    ...
    proto = function_format_protocol( [ opt['protocol'], '6' ] )
